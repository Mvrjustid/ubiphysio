...
args.vq_dir= os.path.join("./dataset/ubiphysio" if args.dataname == 'ubiphysio' else "./dataset/HumanML3D", f'{args.vq_name}')
...
for batch in tqdm(train_loader_token):
    ...
    # pose_modified = torch.cat([pose[..., :283], pose[..., 287:]], dim=-1) # To be used
    target = net.encode(pose)
    target = target.cpu().numpy().astype(int)  # Ensure that all data is integer
    # Save as txt
    np.savetxt(pjoin(args.vq_dir, name[0] +'.txt'), target, fmt='%d')
    # Save as npy
