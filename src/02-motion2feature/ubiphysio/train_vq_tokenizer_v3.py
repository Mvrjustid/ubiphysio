...
def load_models(opt):
    ...
    if opt.is_continue:
    # if True:
        print('Loading Weights from'+pjoin(opt.checkpoints_dir, 't2m',
                                      'VQVAEV3_CB1024_CMT_H1024_NRES3', 'model', 'finest.tar'))
        checkpoint = torch.load(pjoin(opt.checkpoints_dir, 't2m',
                                      'VQVAEV3_CB1024_CMT_H1024_NRES3', 'model', 'finest.tar'),
...
if __name__ == '__main__':
    ...
    elif opt.dataset_name == 'emopain_raw':
        opt.data_root = './dataset/EmoPain/'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
        opt.text_dir = pjoin(opt.data_root, 'texts')
        opt.joints_num = 26
    ...
    print(f"batch size is '{opt.batch_size}'")
